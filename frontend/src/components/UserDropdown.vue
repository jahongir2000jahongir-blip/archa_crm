...
import { showAboutModal, showVisibilityModal } from '@/composables/modals'
...
const MANAGER_ONLY_ITEMS = ['app_selector', 'settings', 'about']
...
const dropdownItems = computed(() => {
  ...
  items.forEach((item) => {
    ...
    if (item.is_standard && MANAGER_ONLY_ITEMS.includes(item.name1) && !isManager()) return
    if (item.type !== 'Separator') {
      _dropdownItems[_dropdownItems.length - 1].items.push(
        dropdownItemObj(item),
      )
      if (item.is_standard && item.name1 === 'about' && isManager()) {
        _dropdownItems[_dropdownItems.length - 1].items.push({
          icon: 'eye',
          label: __('Видимость'),
          onClick: () => (showVisibilityModal.value = true),
        })
      }
    } else {
      _dropdownItems.push({
